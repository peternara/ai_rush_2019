...
def data_generator(input_size=128,
                    batch_size=64,
                    num_workers=0,
                    ):
...
def train_dataloader(input_size=128,
    ...
    data_seed = 777
#    msss = MultilabelStratifiedShuffleSplit(n_splits=1, test_size=0.1, random_state=777)
    train_df, valid_df  = train_test_split(train_meta_data, test_size=0.1, random_state=data_seed)
    ...
    #print(label_matrix)
    train_label, valid_label  = train_test_split(label_matrix, test_size=0.1, random_state=data_seed)
...
class AIRushDataset(Dataset):
    def __init__(self, image_data_path, meta_data,label=None, transform=None, use_multi=True):
